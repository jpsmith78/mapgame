<?php require APPROOT . '/views/inc/header.php'; ?>
  ...
  <div>
    <h1><?php echo ucfirst($_SESSION['user_name']); ?>'s Maps</h1>
    <h5>Map Count: <?php echo count($data['maps']); ?></h5>

    <ul>
      <?php foreach($data['maps'] as $map) : ?>
        <?php $states[] = $map->state_abbr; ?>
        <li><?php echo $map->state_abbr; ?></li>
      <?php endforeach; ?>
    </ul>
    <form id="add_state_input"  action="<?php echo URLROOT; ?>/maps/add" method="post">
      <input id="add_input_value" type="hidden" name="state" value="">
    </form>
    <form id="delete_state_input"  action="<?php echo URLROOT; ?>/maps/delete" method="post">
      <input id="delete_input_value" type="hidden" name="state" value="">
    </form>
    <svg xmlns="http://www.w3.org/2000/svg" width="959" height="593">

    <g class="state" onclick="toggleVisited()">
